...
Original file is located at
    https://colab.research.google.com/drive/1wmyL5LJbrExDHxp35LwQMsjaUKHE2GO3
"""
...
url='https://raw.githubusercontent.com/tiwariabhishekk10/abhishek/master/Google_Stock_Prices.csv'
...
regressor_lstm.add(LSTM(units = 20,return_sequences = True, input_shape = (X.shape[1], 5)))
...
regressor_lstm.add(Dropout(0.5))
regressor_lstm.add(Dense(units = 5))
...
regressor_gru.add(GRU(units = 20, return_sequences = True, input_shape = (X.shape[1], 5)))
...
model.add(Conv1D(filters=20, kernel_size=2, activation='relu', input_shape=(X.shape[1],5)))
...
#Fitting the model
history_cnn = model.fit(X, y, epochs=10, batch_size=20, validation_split=0.1,  shuffle=False)
...
regressor.add(Dense(units = 200, activation='relu', input_shape = (X.shape[1], 5)))
